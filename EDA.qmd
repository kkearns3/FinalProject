...
- Numeric fields
    ...
    + PhysHlth: days of physicial illness or injury in the last 30 days
...
```{r}
g <- ggplot(data = diabetes, aes(x = as.numeric(GenHlth)))
g + geom_histogram(aes(fill = Diabetes_binary, alpha = 0.5))

```


```{r}
g <- ggplot(data = diabetes, aes(x = PhysHlth, y = BMI)) 

g + geom_point(aes(color = Diabetes_binary), position = "jitter")

# diabetes |>
#   select(where(is.numeric))
```

Well that looks insane. Lol.

BMI seems kind of off
```{r}
